...
import useEVCars, { type CarQuery } from '../../hooks/cars/useEVCars';
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className={`mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
            {searchQuery 
              ? `${cars?.length || 0} result${cars?.length !== 1 ? 's' : ''} found`
              : `Browse ${cars?.length || 0} electric vehicle${cars?.length !== 1 ? 's' : ''}`
            }
          </p>
        </div>
          ...
          {totalPages > 1 && (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-6 border-t border-gray-200 dark:border-gray-700">
              <div className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                Showing {startIndex + 1} - {Math.min(endIndex, cars.length)} of {cars.length}
              </div>
